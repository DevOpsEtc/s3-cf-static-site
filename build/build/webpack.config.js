...
PATH.assets = path.resolve(PATH.base, 'assets');
PATH.fonts = path.resolve(PATH.assets, 'fonts');
PATH.images = path.resolve(PATH.assets, 'images');
PATH.scripts = path.resolve(PATH.assets, 'scripts');
PATH.styles = path.resolve(PATH.assets, 'styles');
PATH.exclude = /(node_modules|bower_components)/;
PATH.externals = {};
PATH.entry = { bundle: './assets/index.js' };
...
const makeLoaders = env => [
  ...
  },
  {
    ...
    include: [
      PATH.scripts,
      path.resolve(PATH.assets, 'index.js'),
    ],
    exclude: PATH.exclude,
  },
  {
...
const makePlugins = (env) => {
  const basePlugins = [
    ...
    new CopyPlugin([
      path.resolve(PATH.images, 'favicons/favicon.ico'),
      {
        from: path.resolve(PATH.images, '**/*.{png,jpg,jpeg,gif,webp,svg}'),
        to: 'img',
        flatten: true,
        cache: env.isDev,
      },
